...
def job():
    all_income_automatically=IncomeObject.objects.filter(status=True,is_active=True,added=False)
        ...
        for user_income in all_income_automatically:
            if user_income.next_date == datetime.now().date():
                ...
                new_income=IncomeObject.objects.create(source=user_income.source, amount=user_income.amount, frequency=user_income.frequency, last_date=datetime.date.today(),
                                             ...
                                             added=False,is_active=user_income.is_active)
                ...
                try :
                    ...
                    log.write(f"Email not sent to {user.email} for income {datetime.now()}\n")
